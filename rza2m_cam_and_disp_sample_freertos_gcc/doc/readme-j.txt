...
*             : Dec. 28,2018 Rev.1.01.00    Terminal Appの設定についての記載を削除
*             : Apr. 15,2019 Rev.1.02.00    「ディレクトリ内容」を削除
*             : Dec. 17,2019 Rev.1.03.00    「2. 動作確認環境」を更新
*                                           「3. サンプルコードの内容」を更新
*                                           「5. サンプルコードの動作手順」を更新
***********************************************************************************/
...
3. サンプルコードの内容
  ...
  (1) アプリケーションプログラム [rza2m_cam_and_disp_sample_freertos_gcc]
    MIPIカメラ入力画像を、Displayに出力します。
    また、VDCの画像調整機能(コントラスト、ブライトネスなど)をTerminal Applicationから
    操作することができます。
...
5. サンプルコードの動作手順
  ...
  (2) サンプルコードのセットアップ
    [rza2m_cam_and_disp_sample_freertos_gcc] のディレクトリをホストPCの
    e2 studioワークスペースディレクトリ(例: "C:\e2studio_Workspace_v760")にコピーします。

  (3) 統合開発環境の起動
    統合開発環境e2 studioを起動します。

  (4) アプリケーションプログラム([rza2m_cam_and_disp_sample_freertos_gcc]プロジェクト)のビルド
    e2 studioメニューより、[rza2m_cam_and_disp_sample_freertos_gcc]プロジェクトをインポートした後、
    rza2m_cam_and_disp_sample_freertos_gcc.elfを生成します。
  ...
  (6) サンプルコードのダウンロード
    ...
    アプリケーションプログラムのデバッグ構成[rza2m_cam_and_disp_sample_freertos_gcc HardwareDebug]を選択し、
  ...
  (8) サンプルコードの操作
    ターミナルコンソールには、サブボード上のCN5マイクロUSB接続を介してアクセスできます。
    詳細については、『Camera and Display sampleアプリケーションノート(R01AN4477)』
    を参照してください。
